...
%nodeAttr = gpuArray(dlarray([x; y; Ux0; Uy0; Vx0; Vy0], "CUB"));
...
edgeAttr = gpuArray(dlarray([direc; direc_norm], "CUB"));
edgeMask = gpuArray(dlarray(ones(1, size(srlist,2)), "CUB"));
srlist = gpuArray(dlarray(srlist, "CUB"));
edgesNodesMatrix = gpuArray(dlarray(edgesNodesMatrix, "CUB"));
nodeMask = gpuArray(dlarray(ones(1, numnp), "CUB"));

dirVecU = gpuArray(dlarray(zeros(numnp, 1)));
dirVecV = gpuArray(dlarray(zeros(numnp, 1)));
K_in = gpuArray(dlarray(K_uu));
F_in = gpuArray(dlarray(F_uu));
UV_fem_in = gpuArray(dlarray(UV_fem));
